						...
						<template v-if="!item.hidden">
							<el-submenu :index="index+''" v-if="!item.leaf" :key="index">
								...
								<el-menu-item v-for="child in item.children" :index="child.path" :key="child.path">
									<template v-if="!child.hidden">{{child.name}}</template>
									</el-menu-item>
							</el-submenu>
	...
	export default {
		...
		mounted() {
			...
			if (user) {
				...
				this.sysUserAvatar = user.avatar || 'http://app-jx.oss-cn-beijing.aliyuncs.com/webStorage/web_15547956155764870.png';
			}
