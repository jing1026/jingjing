	...
	<section>
		...
		<el-table :data="list" stripe highlight-current-row v-loading="listLoading" style="width: 100%;">
			...
			<el-table-column prop="url" label="图片">
                <template scope="scope">
					<img class="covers" :src="scope.row.url" />
				</template>
            </el-table-column>
		...
		<el-dialog title="编辑" :visible.sync="addFormVisible" :close-on-click-modal="false" width="500px">
			<el-form :model="addForm" label-width="80px" ref="addForm">
				<el-form-item label="图片" prop="url" :rules="[{ required: true, message: '请选择图片', trigger: 'blur' }]">
					<el-upload class="avatar-uploader photo" :http-request="Upload" :multiple="false" :show-file-list="false" action="" accept="image/jpeg,image/png">
	...
	export default {
		...
		methods: {
			...
			getData:function (){
				...
				this.$http.post('mroom/photo/list', this.qs.stringify(para)).then(data => {
					let  res = data.data;
					if (res.status !== 0) {
						this.$message({
							message: res.msg,
			...
			handleDel(id) {
				this.$confirm('确认删除吗?', '提示', {}).then(() => {
					this.listLoading = true;
					this.$http.post('mroom/photo/delete', this.qs.stringify({id: id})).then(data => {
