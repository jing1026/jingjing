<template>
	<section v-loading="listLoading">
		...
		<el-row>
            ...
            <el-col :span="6">{{data.type === -1 ? '寻物' :'寻人'}}</el-col>
        </el-row>
		<el-row>
	...
	export default {
		...
		methods: {
			...
			getData:function (){
				...
				this.$http.post('mcharity/info', this.qs.stringify({id: this.$route.query.id})).then(data => {
            ...
            examine(para){
				this.listLoading = true;
				this.$http.post('mcharity/update', this.qs.stringify(para)).then(data => {
			...
			adopt() {
				this.$confirm('确认通过该记录吗?', '提示', {}).then(() => {
					let para = { charityIds: this.data.id,status: 1};
					this.examine(para);
				}).catch(() => {
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							let para = { charityIds: this.data.id, status: -1,reason: this.editForm.reason};
							this.examine(para);
