	...
	export default {
		...
		methods: {
			...
			getData:function (){
				...
				this.$http.post('msubject/list', this.qs.stringify(para)).then(data => {
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.$http.post('msubject/add', this.qs.stringify(para)).then(data => {
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.$http.post('msubject/update', this.qs.stringify(this.editForm)).then(data => {
			...
			handleDel: function (index, row) {
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					this.listLoading = true;
					let para = { id: row.id };
					this.$http.post('msubject/delete', this.qs.stringify(para)).then(data => {
