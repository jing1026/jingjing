<template>
	<section>
		...
		<el-table :data="list" stripe highlight-current-row v-loading="listLoading" style="width: 100%;">
			...
			<el-table-column prop="content" label="内容">
				<template scope="scope">
					<div class="maxH" v-html="scope.row.content"></div>
				</template>
			</el-table-column>
		...
		<!--编辑界面-->
		<el-dialog title="编辑" :visible.sync="addFormVisible" :close-on-click-modal="false" @close="dialogClose" width="620px">
		...
		methods: {
           ...
           dialogClose(){
			...
			getData:function (){
				...
				this.$http.post('mfeatrue/get').then(data => {
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							this.$http.post('mfeatrue/update', this.qs.stringify(this.addForm)).then(data => {
