<template>
	<section>
		...
		<el-col :span="24" class="toolbar">
			...
			<el-button size="small" type="danger" @click="batchIgnore" :disabled="this.sels.length===0">批量忽略</el-button>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//批量忽略
			batchIgnore(id) {
				this.$confirm('确认忽略吗?', '提示', {}).then(() => {
					this.listLoading = true;
					this.$http.post('mcircle/remove', this.qs.stringify({circleIds: id,status: -1, adminId: sessionStorage.getItem('id')})).then(data => {						
						let  res = data.data;
						if (res.status !== 0) {
							this.$message({
								message: res.msg,
