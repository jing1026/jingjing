<template>
	<section v-loading="listLoading">
        ...
        <el-row>
            <el-col :span="3">实拍图:</el-col>
            <el-col :span="20">
                <el-row v-if="data.realPhoto">
                    <el-col :span="6" v-for="item in data.realPhoto.split(',')" :key="item.index">
                        <img :src="item" />
                    </el-col>
                </el-row>
            </el-col>
	...
	export default {
		...
		methods: {
			...
			getData:function (){
				...
				this.$http.post('mused/info', this.qs.stringify({id: this.$route.query.id})).then(data => {
            ...
            examine(para){
				this.listLoading = true;
				this.$http.post('mused/update', this.qs.stringify(para)).then(data => {
			...
			adopt() {
				this.$confirm('确认通过该记录吗?', '提示', {}).then(() => {
					let para = { usedIds: this.data.id,status: 1, adminId: sessionStorage.getItem('id')};
					this.examine(para);
				}).catch(() => {
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							let para = { usedIds: this.data.id, status: -1,reason: this.editForm.reason, adminId: sessionStorage.getItem('id')};
							this.examine(para);
