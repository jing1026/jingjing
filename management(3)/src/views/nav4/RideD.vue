<template>
	<section v-loading="listLoading">
        <el-row style="margin-top:30px;">
            ...
            <el-col :span="4">发布者昵称: {{data.nickname}}</el-col>
            <el-col :span="6">时间: {{data.createTime}}</el-col>
        </el-row>
        <el-row style="margin-top:30px;">
            <el-col :span="3">出发地:</el-col>
            <el-col :span="9">{{data.startPlace}}</el-col>
        </el-row>
        <el-row>
            <el-col :span="3">目的地:</el-col>
            <el-col :span="9">{{data.endPlace}}</el-col>
        </el-row>
        <el-row>
            <el-col :span="3">出发时间:</el-col>
            <el-col :span="9">{{data.startTime}}</el-col>
        </el-row>
        <el-row>
            <el-col :span="3">座位数:</el-col>
            <el-col :span="9">{{data.seatNum}}</el-col>
        </el-row>
        <el-row>
            <el-col :span="3">描述:</el-col>
            <el-col :span="9" v-html="data.inform.replace(/\r\n/g, '<br/>').replace(/\n/g, '<br/>').replace(/\s/g, ' ')"></el-col>
        </el-row>
        <el-row>
            <el-col :span="3">联系电话:</el-col>
            <el-col :span="9">{{data.phone}}</el-col>
        </el-row>
        <el-row>
            <el-col :span="3">图片:</el-col>
            <el-col :span="20">
                <el-row v-if="data.carPhoto">
                    <el-col :span="6" v-for="item in data.carPhoto.split(',')" :key="item.index">
                        <img :src="item" />
                    </el-col>
                </el-row>
            </el-col>
	...
	export default {
		...
		methods: {
			...
			getData:function (){
				...
				this.$http.post('mcar/info', this.qs.stringify({id: this.$route.query.id})).then(data => {
            ...
            examine(para){
				this.listLoading = true;
				this.$http.post('mcar/update', this.qs.stringify(para)).then(data => {
