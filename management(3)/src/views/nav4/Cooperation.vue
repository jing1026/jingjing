	...
	<section>
		...
		<el-table :data="list" stripe highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="introduce" label="介绍">
				<template scope="scope">
					<div v-html="scope.row.introduce.replace(/\r\n/g, '<br/>').replace(/\n/g, '<br/>').replace(/\s/g, ' ')"></div>
				</template>
			</el-table-column>
		...
		<el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false" @close="dialogClose" width="620px">
            <el-form :model="addForm" label-width="80px" ref="addForm">
                ...
                <el-form-item label="合作名称" prop="name" :rules="[{ required: true, message: '请填写合作名称', trigger: 'blur' }]">
				...
				<el-form-item label="合作简介" prop="introduce" :rules="[{ required: true, message: '请填写合作简介', trigger: 'blur' }]">
					<el-input type="textarea" :autosize="{ minRows: 2, maxRows: 4}" v-model="addForm.introduce" auto-complete="off"></el-input>
				</el-form-item>
	...
	export default {
		...
		methods: {
			...
			getData:function (){
				...
				this.$http.post('mcollaborate/list', this.qs.stringify(para)).then(data => {
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.$http.post('mcollaborate/add', this.qs.stringify(this.addForm)).then(data => {
			...
			editSubmit: function () {
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							this.listLoading = true;
							this.$http.post('mcollaborate/update', this.qs.stringify(this.editForm)).then(data => {
			...
			handleDel(id) {
				this.$confirm('确认删除吗?', '提示', {}).then(() => {
					this.listLoading = true;
					this.$http.post('mcollaborate/delete', this.qs.stringify({ids: id, adminId: sessionStorage.getItem('id')})).then(data => {
