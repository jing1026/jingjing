	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
                ...
                <el-form-item>
					<el-select v-model="filters.type" placeholder="分类">
		...
		<el-table :data="list" stripe highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="parentType" :formatter="parentType" label="店铺分类"></el-table-column>
            <el-table-column prop="tradeMethod" label="交易方式"></el-table-column>
			...
			<el-table-column prop="createTime" label="发布时间" sortable min-width="130"></el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
            add(){
				this.$router.push({path:'/marketAdd' });
			},
			status: function (row, column) {
				return row.status === 0 ? '审核中' : row.status === 1 ? '审核通过' : '未通过';
			},
			parentType(row, column) {
				return row.parentType === 0 ? '二手手机' : row.parentType === 1 ? '二手车':row.parentType === 2 ? '二手数码' : row.parentType === 3 ? '其他二手'  : '';
			},
			...
			getData:function (){
				...
				this.$http.post('mused/list', this.qs.stringify(para)).then(data => {
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							this.listLoading = true;
							this.$http.post('mused/update', this.qs.stringify(this.editForm)).then(data => {
			...
			handleDel(id) {
				this.$confirm('确认删除吗?', '提示', {}).then(() => {
					this.listLoading = true;
					this.$http.post('mused/delete', this.qs.stringify({ids: id, adminId: sessionStorage.getItem('id')})).then(data => {
